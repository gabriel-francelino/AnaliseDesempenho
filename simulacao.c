...
int main()
{
    ...
    while (tempo_decorrido < params.tempo_simulacao)
    {
        ...
        if (tempo_decorrido == tempo_chegada)
        {
            ...
            // Calcula o tempo de saída do cliente se a fila estiver vazia.
            if (!fila)
            {
            ...
            e_n.soma_areas += (tempo_decorrido - e_n.tempo_anterior) * e_n.num_eventos;
            e_n.num_eventos++;
            e_n.tempo_anterior = tempo_decorrido;

            e_w_chegada.soma_areas += (tempo_decorrido - e_w_chegada.tempo_anterior) * e_w_chegada.num_eventos;
            e_w_chegada.num_eventos++;
        ...
        else if (tempo_decorrido == tempo_saida)
        {
            ...
            e_w_saida.soma_areas += (tempo_decorrido - e_w_saida.tempo_anterior) * e_w_saida.num_eventos;
            e_w_saida.num_eventos++;
    ...
    double e_w_calculo = (e_w_chegada.soma_areas - e_w_saida.soma_areas) / e_w_chegada.num_eventos;
    double lambda = e_w_chegada.num_eventos / tempo_decorrido;

    printf("Número médio de clientes no sistema. E[N]: %lF\n", e_n_calculo);
    printf("Tempo médio de espera de um cliente no sistema. E[W]: %lF\n", e_w_calculo);
